...
public class TravelPlanService {
  ...
  public TravelPlanResponseDto createTravelPlan(CreateTravelPlanDto dto)
            ...
            .orElseThrow(
                () ->
                    new UsuarioNaoEncontradoException(
                        "Usuário não encontrado com o ID: " + dto.getUserId()));

    TravelPlanEntity travelPlan =
        TravelPlanEntity.builder()
            .title(dto.getTitle())
    ...
    List<DayItineraryEntity> dayItinerary =
        dto.getDayItinerary().stream()
            .map(
                itineraryDto -> {
  ...
  public TravelPlanResponseDto updateTravelPlan(UUID id, CreateTravelPlanDto dto) {
    ...
    List<DestinationEntity> updatedDestinations =
        dto.getDestinations().stream()
            .map(
                destDto ->
    ...
    List<DayItineraryEntity> updateDayItinerary =
        dto.getDayItinerary().stream()
            .map(
                itineraryDto ->
  ...
  private TravelPlanResponseDto toResponseDto(TravelPlanEntity entity) {
    ...
    List<DayItineraryResponseDto> dayItinerary =
        entity.getDayItinerary().stream()
            .map(
                itinerary ->
                    new DayItineraryResponseDto(itinerary.getTitle(), itinerary.getActivities()))
            .collect(Collectors.toList());
