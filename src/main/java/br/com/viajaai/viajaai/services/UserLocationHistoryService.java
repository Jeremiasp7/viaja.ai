...
public class UserLocationHistoryService {
  ...
  public UserLocationHistoryResponseDto addLocationHistory(
      UUID userId, CreateUserLocationHistoryDto dto) {
    UserEntity user =
        userRepository
            .findById(userId)
  ...
  public List<UserLocationHistoryResponseDto> getFavoriteHistoryForUser(UUID userId) {
    return historyRepository.findByUserIdAndIsFavoriteTrue(userId).stream()
        .map(UserLocationHistoryResponseDto::fromEntity)
        .collect(Collectors.toList());
  }

  public List<UserLocationHistoryResponseDto> getVisitedHistoryForUser(UUID userId) {
    return historyRepository.findByUserIdAndHasVisitedTrue(userId).stream()
        .map(UserLocationHistoryResponseDto::fromEntity) // Use Dto static method
        .collect(Collectors.toList());
  }

  public UserLocationHistoryResponseDto updateLocationHistory(
      UUID historyId, CreateUserLocationHistoryDto dto) {
    UserLocationHistory existingHistory =
        historyRepository
            .findById(historyId)
            .orElseThrow(
                () ->
                    new EntityNotFoundException("LocationHistory not found with ID: " + historyId));
