...
public class ChecklistService {
  ...
  public ChecklistResponseDto criarChecklist(CreateChecklistDto dto) {
    TravelPlanEntity travelPlan =
        travelPlanRepository
            .findById(UUID.fromString(dto.getTravelPlanId()))
            .orElseThrow(
                () -> new TravelPlanNaoEncontradoException("Plano de viagem não encontrado"));
  ...
  public List<ChecklistResponseDto> listarPorPlano(UUID travelPlanId) {
    ...
    return checklistRepository.findByTravelPlan(travelPlan).stream()
        .map(this::toDto)
  ...
  private ChecklistResponseDto toDto(ChecklistEntity entity) {
    return new ChecklistResponseDto(
        entity.getId(), entity.getNome(), entity.isConcluido(), entity.getTravelPlan().getId());
  }
}
