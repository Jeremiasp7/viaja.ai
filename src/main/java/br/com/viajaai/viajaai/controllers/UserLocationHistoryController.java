...
public class UserLocationHistoryController {
  ...
  public ResponseEntity<UserLocationHistoryResponseDto> addHistoryToUser(
      @PathVariable UUID userId, @RequestBody CreateUserLocationHistoryDto locationHistoryDTO) {

    UserLocationHistoryResponseDto savedHistory =
        historyService.addLocationHistory(userId, locationHistoryDTO);
    return ResponseEntity.status(HttpStatus.CREATED).body(savedHistory);
  }

  @GetMapping("/users/{userId}/history")
  public ResponseEntity<List<UserLocationHistoryResponseDto>> getAllHistoryForUser(
  ...
  public ResponseEntity<List<UserLocationHistoryResponseDto>> getFavoriteHistoryForUser(
      @PathVariable UUID userId) {
    List<UserLocationHistoryResponseDto> favorites =
        historyService.getFavoriteHistoryForUser(userId);
    return ResponseEntity.ok(favorites);
  }

  @GetMapping("/users/{userId}/history/visited")
  public ResponseEntity<List<UserLocationHistoryResponseDto>> getVisitedHistoryForUser(
      @PathVariable UUID userId) {
  ...
  public ResponseEntity<UserLocationHistoryResponseDto> updateHistory(
      @PathVariable UUID historyId, @RequestBody CreateUserLocationHistoryDto locationHistoryDTO) {

    UserLocationHistoryResponseDto updatedHistory =
        historyService.updateLocationHistory(historyId, locationHistoryDTO);
