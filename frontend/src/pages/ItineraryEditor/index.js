  ...
  useEffect(() => {
    async function loadPlan() {
      ...
      try {
        ...
        setPlan(resp.data);
        setDateForm({ startDate: resp.data.startDate || '', endDate: resp.data.endDate || '' });
      } catch (err) {
      ...
      <header className="planner-header">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <h3 style={{ margin: 0 }}>Roteiro: {plan?.title || 'Novo Roteiro'}</h3>
            {!editingDates && plan?.startDate && (
              <div className="plan-dates">{formatDate(plan.startDate)} — {formatDate(plan.endDate)}</div>
            )}
            {editingDates && (
              <div className="date-edit-inline">
                ...
                <button className="btn-secondary" onClick={() => setEditingDates(false)}>Cancelar</button>
            ...
            {!editingDates && (
              <button className="btn-secondary" onClick={() => setEditingDates(true)}>Editar datas</button>
            )}
          </div>
        </div>
        <div className="tabs">
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <div>
                  <button className="btn-secondary" onClick={async () => {
                    if (!plan) return;
                    const newDay = { title: `Dia ${ (plan.dayItinerary?.length || 0) + 1 }`, activities: [] };
              ...
              {plan?.dayItinerary && plan.dayItinerary.length > 0 ? (
                plan.dayItinerary.map((day, idx) => (
                  <div className="day-card" key={idx}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <h4 style={{ margin: 0 }}>Dia {idx + 1} — {dateForDay(idx)} {day.title ? `- ${day.title}` : ''}</h4>
                      <div>
                        <button className="btn-icon" title="Remover dia" onClick={async () => {
                          if (!plan) return;
                          if (!window.confirm('Remover este dia do itinerário?')) return;
                    ...
                    {day.activities && day.activities.map((act, aIdx) => (
                      <div className="activity-card" key={aIdx}>
                        ...
                        <button className="btn-icon small" title="Remover atividade" onClick={async () => {
                          if (!plan) return;
                    ...
                    {addingActivityFor === idx ? (
                      <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                        <input className="inline-input" value={newActivityText} onChange={(e) => setNewActivityText(e.target.value)} placeholder="Descrição da atividade" />
                        <button className="btn-create" onClick={async () => {
                        ...
                        <button className="btn-secondary" onClick={() => { setAddingActivityFor(null); setNewActivityText(''); }}>Cancelar</button>
                      </div>
                    ) : (
                      <button className="add-activity-btn" onClick={() => setAddingActivityFor(idx)}><FaPlus /> Adicionar Atividade</button>
                    )}
                  </div>
                ))
              ) : (
                <p>Sem itens no itinerário. Adicione destinos ou dias para começar.</p>
              )}
            </>
          )}
        </div>
          ...
          {activeTab === 'ia' && (
            <div className="suggestion-panel">
              <div className="suggestion-controls-top">
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, width: '100%' }}>
                  <div>
                    ...
                    <p style={{ margin: '6px 0 0 0', fontSize: '0.95rem' }}>Gere uma sugestão usando IA.</p>
                  </div>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    <button className="btn-primary" onClick={async () => {
                      setSugError(null); setSugResult('');
                      if (!plan?.id) { setSugError('Nenhum travel plan carregado.'); return; }
                      setSugLoading(true);
                      try {
                        ...
                        setSugError(err.response?.data || err.message || 'Erro ao gerar sugestão por plano');
                      } finally { setSugLoading(false); }
                    }} disabled={sugLoading}>{sugLoading ? 'Gerando...' : 'Gerar por travelPlan'}</button>
                  </div>
                </div>
              </div>
              ...
              <div className="suggestion-result-box">
                ...
                {sugResult ? (
                  <div className="result-card"><pre className="result-pre">{typeof sugResult === 'string' ? sugResult : JSON.stringify(sugResult, null, 2)}</pre></div>
                ) : (
