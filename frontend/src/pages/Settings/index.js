...
export default function Settings() {
  ...
  const addDate = () => {
    if (!dateInput) return;
    setForm((f) => ({ ...f, preferenciaDeDatas: [...f.preferenciaDeDatas, dateInput] }));
    setDateInput('');
  };

  const removeDate = (idx) => {
    setForm((f) => ({ ...f, preferenciaDeDatas: f.preferenciaDeDatas.filter((_, i) => i !== idx) }));
  };
  ...
  const handleSave = async (e) => {
    ...
    try {
      const payload = {
        ...form,
        duracaoDaViagem: form.duracaoDaViagem ? Number(form.duracaoDaViagem) : null,
      };
  ...
  return (
      ...
      <div className="settings-grid">
        <form className="prefs-form" onSubmit={handleSave}>
          <label>Estilo de Viagem
            <input name="estiloDeViagem" value={form.estiloDeViagem} onChange={handleChange} placeholder="Ex: Aventura, Relax" />
          </label>

          <label>Preferência de Acomodação
            <input name="preferenciaDeAcomodacao" value={form.preferenciaDeAcomodacao} onChange={handleChange} placeholder="Ex: Hotel, Hostel, Airbnb" />
          </label>

          <label>Preferência de Clima
            <input name="preferenciaDeClima" value={form.preferenciaDeClima} onChange={handleChange} placeholder="Ex: Quente, Frio" />
          </label>

          <label>Faixa Orçamentária
            <input name="faixaOrcamentaria" value={form.faixaOrcamentaria} onChange={handleChange} placeholder="Ex: baixo, médio, alto" />
          </label>

          <label>Duração da Viagem (dias)
            <input name="duracaoDaViagem" type="number" min="1" value={form.duracaoDaViagem} onChange={handleChange} />
          </label>

          <label>Companhia de Viagem
            <input name="companhiaDeViagem" value={form.companhiaDeViagem} onChange={handleChange} placeholder="Ex: sozinho, casal, família" />
          </label>
          ...
          <div className="dates-block">
            ...
            <ul className="dates-list">
              {form.preferenciaDeDatas.map((d, i) => (
                <li key={i}>{d} <button type="button" onClick={() => removeDate(i)}>Remover</button></li>
              ))}
            </ul>
          </div>
        ...
        <aside className="prefs-suggestion">
          ...
          <div className="suggestion-actions">
            <button className='buttonSubmit' onClick={handleGenerateSuggestion} disabled={loading}>{loading ? 'Gerando...' : 'Gerar sugestão para mim'}</button>
          </div>
          <div className="suggestion-result">
            {suggestion ? <pre className="result-pre">{suggestion}</pre> : <div className="hint">Nenhuma sugestão gerada ainda.</div>}
          </div>
        </aside>
      </div>
    </div>
    ...
    <footer className="app-footer">
      <div className={`footer-icon ${isActive('/dashboard') ? 'active' : ''}`} onClick={() => navigate('/dashboard')}><FaHome /><p>Início</p></div>
      <div className={`footer-icon ${isActive('/itinerary') ? 'active' : ''}`} onClick={() => navigate('/dashboard')}><FaSuitcase /><p>Roteiros</p></div>
      <div className={`footer-icon ${isActive('/favoritos') ? 'active' : ''}`}><FaHeart /><p>Favoritos</p></div>
      <div className={`footer-icon ${isActive('/ajustes') ? 'active' : ''}`} onClick={() => navigate('/ajustes')} title="Preferências"><FaCog /><p>Preferências</p></div>
    </footer>
    </>
  );
}
