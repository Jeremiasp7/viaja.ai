...
import { FaSearch, FaHome, FaHeart, FaSuitcase, FaCog, FaMapMarkerAlt, FaCalendarAlt } from 'react-icons/fa';
...
const Dashboard = () => {
  ...
  const calcularProgresso = (startDate, endDate) => {
    ...
    //console.log('Calculando progresso para roteiro de', inicio, 'a', fim, 'hoje é', hoje);
  ...
  return (
    <div className="home-screen">
      ...
      <main className="container">
        <section className="trips-section">
          ...
          <div className="cards-container">
            {roteiros.length > 0 ? (
              <>
                {roteiros.slice(-3).reverse().map((roteiro) => {
              ...
              <p className="no-trips">Você ainda não possui roteiros criados.</p>
            )}
          </div>
        </section>
      ...
      <footer className="app-footer">
        <div className="footer-icon active" onClick={() => navigate('/')}><FaHome /><p>Início</p></div>
        <div className="footer-icon" onClick={() => navigate('/roteiros')}><FaSuitcase /><p>Roteiros</p></div>
        <div className="footer-icon" onClick={() => navigate('/favoritos')}><FaHeart /><p>Favoritos</p></div>
        <div className="footer-icon" onClick={() => navigate('/ajustes')}><FaCog /><p>Ajustes</p></div>
      </footer>
    </div>
  );
};
